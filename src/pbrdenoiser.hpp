    ...
    UT_PtrArray<PXL_Raster *> raster_array) 
{
        ...
        IMG_File *output_file = IMG_File::create(filename, (const IMG_Stat)output_stat, &parms);
...
std::unique_ptr<IMG_File> read_rasters_as_float(const char * filename, 
        UT_PtrArray<PXL_Raster *> & raster_array)
{
...
namespace intel 
{
std::unique_ptr<float[]> filter_with_oidn(void * color, void * albedo, void * normals,
    const char** error, const int xres, const int yres, const bool hdri, const bool srgb)
{
    ...
    if (albedo) {
        filter.setImage("albedo", albedo, oidn::Format::Float3, xres, yres); // optional
    }
    if (normals) {
        filter.setImage("normal", normals, oidn::Format::Float3, xres, yres); // optional
    }

    filter.setImage("color",   color,               oidn::Format::Float3, xres, yres);
    filter.setImage("output",  (void*)output.get(), oidn::Format::Float3, xres, yres);
