...
{
public:
    AccumulateParallel(const PXL_Raster *raster, const int width, 
        const int samplex, const int sampley, T * accumulator):
            raster(raster),
...
void accumulateThreaded(const UT_BlockedRange<int> &range, const PXL_Raster *raster, \
    const int width, const int sx, const int sy, T *accumulator)
{
